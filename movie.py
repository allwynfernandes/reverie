...
def make_montage(camVidSize):
    now = datetime.datetime.now().strftime("%Y-%m-%d_%H_%M")
    print(now)
    scrClip = mpy.ImageSequenceClip("./data/scr/", fps=24)
    camClip = mpy.ImageSequenceClip("./data/cam/", fps=24)
    ...
    audioClip = mpy.AudioFileClip("./data/snd/dirty_dubstep.wav")
